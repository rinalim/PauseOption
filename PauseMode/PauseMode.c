		...
		while (1) {
			...
			if( SELECT_BTN_ON == 1 && START_BTN_ON == 1 )
			{
				if( PAUSE_MODE_ON == 0)
				{
					...
					SELECT_BTN_ON = 0;
					START_BTN_ON = 0;
					FILE *fp = fopen("/dev/shm/runcommand.log", "r");
					char temp[255];
					int i = 0;
					while(1){
					...
					fclose(fp);
					//if(!strcmp(temp, "bublbobl"))
					if(!strcmp(temp, "1944\n"))
						sprintf(cmd, "%s -b0x0000 -l30000 %s &", PATH_PNGPAUSE, "/home/pi/PauseMode/PauseMode/pause-bublbobl.png");
					else
						sprintf(cmd, "%s -b0x0000 -l30000 %s &", PATH_PNGPAUSE, PATH_PAUSE_RESUME);
					system(cmd);
